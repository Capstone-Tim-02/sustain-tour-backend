...
func GetEmailBody(ticket model.Ticket, totalCost int, wisataName, kodeVoucher string, pointsEarned, usedPoints int, carbonFootprint float64) string {
	...
	emailBody += "<h1>Pembelian Tiket Wisata Berhasil</h1>"
	emailBody += "<p>Terima kasih telah melakukan pembelian tiket wisata. Berikut adalah rincian pembelian Anda:</p>"
	emailBody += "<div class='invoice-details'>"
	emailBody += "<p><strong>Nomor Invoice:</strong> " + ticket.InvoiceNumber + "</p>"
	emailBody += "<p><strong>Nama Tempat Wisata:</strong> " + wisataName + "</p>"
	...
	emailBody += "<p><strong>Carbon Footprint:</strong> " + fmt.Sprintf("%.2f", carbonFootprint) + " grams CO2</p>" // Menambahkan carbon footprint di sini
	emailBody += "</div>"
	emailBody += "<hr>"
	emailBody += "<p>Terima kasih atas pembelian Anda!</p>"
	emailBody += "<p>Kami berharap Anda memiliki pengalaman yang menyenangkan di menggunakan platform kami.</p>"
